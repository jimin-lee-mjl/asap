...
for s in result['reviewText']:
  clean_text.append(preprocess_sentence(s))
      # clean_text.append(str(s.index()))
      # NaN이 float로 인식되기 때문에 
clean_text[:5]
...
result = result.copy() # 왜 copy를 해서 [A value is trying to be set on a copy of a slice from a DataFrame. Try using .loc[row_indexer,col_indexer] = value instead] 에러를 해결해야 하는가?
result['reviewText'] = clean_text
...
result['decoder_input'] = result['summary'].apply(lambda x : 'sostoken '+ x)
